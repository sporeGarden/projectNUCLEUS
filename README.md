...
- **Continuous membrane telemetry**: `nucleus-deploy telemetry` probes both membranes (VPS + gate) every 15 min via cron. `nucleus-deploy summary` produces rolling 7-day `validation/baselines/membrane_7day.toml` with parity checks and cutover gates. (Replaces `deploy/membrane_telemetry.sh` and `deploy/membrane_summary.sh`, now in `deploy/legacy/`.) Shadow data is **permanent** — collection continues beyond cutover
