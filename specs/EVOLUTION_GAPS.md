...
**Last updated**: 2026-05-27 (Wave 56 absorption — 460 methods, `--uds-only` VPS standard, cell graph tagging, NC-1→NC-5 niche climate)
...
**Tier 2 Science API**: `toadstool.validate` IMPLEMENTED (S250, 74 methods), `barracuda.precision.route` v0.4.0 (649 tests), `composition.deploy.shadow` biomeOS v3.53 SHIPPED, `biomeos.spring_status` IMPLEMENTED (v3.78). Registry at **460 methods** (Wave 56 — `nucleus.ingest_spore` + `nucleus.emit_spore` added; typed `DispatchError`/`IonicProtocolError`/`PhasedIpcError` thiserror-derived). Canonical `primal.list` / `capability.list` schemas SHIPPED. `env_keys.rs` centralization. biomeOS v3.77 scaffolded `nucleus ingest/emit` gateway (NC-1); v3.78 deep debt cleanup. **Wave 56**: `nucleus_launcher --uds-only`, cell graph `vps_standard` tagging, 12 primordial scripts archived, `primalspring checksums` + `primalspring registry` subcommands replace shell validation.
...
| H3-04 | GitHub repos (source hosting) | Forgejo primary, GitHub mirror | **ACTIVE** — 32 repos mirrored, dual-push configured | Forgejo Actions working |
