...
**Last updated**: 2026-08-10 (Wave 157g ENMESH — biome.yaml manifest validation shipped, RPC surface audit, gossip reachability documented. 0 P0, 0 P1, 2 P2. All previous closures absorbed.)
**Validation baseline**: 267 PASS, 0 FAIL, 0 KNOWN_GAP (bash 5-layer) + 33 PASS Dark Forest gate. **274 Rust tests** (darkforest 149, tunnelKeeper 48+1 ignored, nucleus-deploy 58, nucleus-primals 19), coverage: darkforest 40.77%, tunnelKeeper 52.67%
**Rust validator**: darkforest v3.0 — 14 modules (8 inner + 6 outer), `--scope inner|outer|full`, `--suite observer` static surface validation. **Outer membrane live**: **26/26 PASS** against primals.eco (TLS 1.3, HSTS, headers, depot, forge, DNS+DNSSEC, mesh)
...
**Tier 2 Science API**: `toadstool.validate` IMPLEMENTED (S325, 112 methods, 9,074 tests), `barracuda.precision.route` v0.4.0 (649 tests), biomeOS v4.56 (8,351 tests, 88% coverage). Registry at **502+ methods** (Wave 136a). Pepti 100% — 34/34 builds, 0 failures, 16 binaries × 2 triples. WAN-DISPATCH-01 transport PASS (10/10, 142ms p50). DNS cutover COMPLETE — site LIVE + HARDENED (EXP-01/02/03/04 patched). darkforest v3.0 outer membrane scope shipped (26 checks, 146 tests). Cooling sprint continues. primalSpring v0.9.39 — 167 scenarios, KNOWN_DEBT calibrated. Neural API LIVE (48 primals discovered). 3 live public surfaces (sporePrint P0 404 divergence on golgi). 55 depot binaries (4 architectures, BLAKE3 + Ed25519 signed), re-harvest → 56. Silicon Atheism Phase 1 complete, Phase 2 active (abstraction over gating).
...
**Rust evolution**: `validation/darkforest/` v3.0 — modular auditable security framework. `rustls` + `webpki-roots` (ring backend) for HTTPS probes. 14 source modules: **inner** (8): `check.rs` (structured types + env-var-driven primal config),
`net.rs` (TCP/HTTP/UDP/HTTPS helpers with DNS resolution), `pentest.rs` (3 threat actors), `fuzz.rs` (14 primals + JupyterHub),
...
|----|-----|-------|--------|--------|
| NC-1 | postPrimordial spore gateway (`biomeos nucleus ingest/emit`) | biomeOS + lithoSpore | **CODE COMPLETE** — biomeOS v4.56 shipped `biomeos-pseudospore` + emit materialization. NC-1.3 COMPLETE. NC-1.4 RESOLVED. Live column U gated on VPS deploy (P0) + 2 spring column U passes. | Stadial entry; columns U/V/W |
...
| `darkforest` (Rust) | `validation/darkforest/` | 14-module security validator (8 inner + 6 outer), 149 tests | JSON report, per-check severity |
...
Niche climate (Wave 59):          ████░░░░░░  NC-1 CODE COMPLETE (live gated on VPS v4.56). NC-2 IN PROGRESS. NC-3 CODE CONSUMED. NC-4 ADVANCING.
...
| 2026-07-11 | **Wave 136b cascade — DNSSEC + K-Derm + 26/26 clean sweep**: (1) **ODN-02 RESOLVED**: DNSSEC live via Cloudflare + Porkbun DS (keyTag 2371, alg 13 ECDSAP256SHA256). darkforest re-scan: **26/26 PASS, 0 FAIL, 0 DARK_FOREST** — clean sweep. Duration improved 23.8s→13.8s. (2) **K-Derm reaffirmed**: Cloudflare IS the outer membrane (trans layer), Caddy is inner (cis). This is the target architecture, not transitional. SURGE-01 dropped — CF IS the CDN. (3) **Docs aligned**: README.md, PHASES.md, SECURITY_VALIDATION.md, EVOLUTION_GAPS.md, DF-REPORT all updated for K-Derm topology and 26/26 result. (4) **primalSpring v0.9.35**: 132 scenarios, 1,104 tests. 12 repos evolved across 3 cascades. |
| 2026-07-12 | **Wave 137b — Neural API LIVE + overwatch split**: (1) **Neural API LIVE** on sporeGate — 19 primals discovered, capability routing verified, depot cryptographically signed (SIGN-01 E2E). (2) **FLOCKGATE-MESH resolved** — port 8080→7700 (`f05918a`), 4 overlay peers from flockGate. (3) **ironGate overwatch split** — projectNUCLEUS = code only; ironGate hardware+deployments moved to dedicated overwatch agent. (4) **Ecosystem 3,960+ tests** / 0 fail across 6 suites (cellMembrane 1,024 added). (5) **deny.toml 4/4** — `cargo deny check` PASS on all crates (v0.19 format, AGPL + CDLA-Permissive-2.0 allowed). (6) **primalSpring v0.9.36** — `445826b`, 141 scenarios. (7) **NUCLEUS-MATRIX**: Columns U/V/W (spore ingest/emit/profile) defined in TRANSPORT_MATRIX.md. (8) **BOND-METADATA**: `bond_type` added to all 19 deployment graphs (covalent/metallic/ionic taxonomy). |
| 2026-07-11 | **Wave 137a — Convergence + deploy.rs split**: (1) **CONVERGED**: All 8 stadial criteria clear. 2,930+ tests / 0 fail across 5 suites (primalSpring 1,125, groundSpring 1,047+, skunkBat 563, projectNUCLEUS 149, footPrint 46). 14/14 primals zero-debt. 40 repos converged. (2) **deploy.rs split**: `deploy/nucleus-deploy/src/deploy.rs` (885L) split into `deploy.rs` (620L) + `process.rs` (269L). Per-primal process management (PrimalContext, start_primal, configure_primal_cmd, verify_primals, hostname) extracted to `process.rs`. Zero test changes — 47 PASS. (3) **Test audit**: 256 Rust tests across 4 crates (darkforest 149, tunnelKeeper 48, nucleus-deploy 47, nucleus-primals 12). Zero clippy warnings, zero fmt issues, zero TODO/FIXME. (4) **primalSpring fb03030** synced — `load_checksums` nested arch-keyed fix absorbed. wateringHole HEAD updated. |
| 2026-07-11 | **Wave 136b — DF-REPORT + footPrint composition**: (1) **DF-REPORT published**: `specs/DARKFOREST_OUTER_MEMBRANE_REPORT.md` — formal execution report from live darkforest v3.0 outer membrane scan. JSON artifact: `validation/reports/outer_membrane_136b.json`. Results stable: 25 PASS, 0 FAIL, 1 DARK_FOREST (ODN-02 DNSSEC). Duration 23.8s, TLS13_AES_128_GCM_SHA256. (2) **footPrint composition target**: `graphs/footprint_composition.toml` — first protist (application composition). petalTongue serves Leaflet/Turf.js frontend, nestGate replaces Express CRUD with CAS persistence (rootPulse-traced), songBird drawbridge replaces Express `/api/proxy` (allowlist: OSM, FEMA, USGS, ArcGIS). Express server disappears. `application = true` fragment metadata. (3) **graphs/README.md**: New "Application Compositions (protists)" section added. (4) **All 8 stadial criteria clear**. Outer membrane sprint complete (9/14 exposures closed). primalSpring 129 scenarios / 1102 tests. |
| 2026-07-10 | **Wave 136c — Outer membrane executed**: (1) **darkforest TLS evolution**: Added `rustls` + `webpki-roots` (ring crypto backend, `default-features = false`) to `net.rs` — `https_get()`, `https_method()`, `tls_probe()` functions with DNS resolution via `ToSocketAddrs`. All 6 outer modules evolved from plaintext TCP to real TLS handshakes. (2) **DNS fixes**: AXFR probe fixed (removed broken `@` nameserver arg), NXDOMAIN timeout handling improved. (3) **Live scan** (`darkforest --scope outer --target primals.eco`): **25 PASS, 0 FAIL, 0 KNOWN_GAP, 1 DARK_FOREST** (ODN-02: DNSSEC not enabled). TLS 1.3 negotiated, HSTS preload confirmed, all security headers present, proper 404, verb blocking (405), path traversal blocked, depot reachable with checksums (3632 bytes), Forgejo SSH key-only (SSH-2.0-Go), AXFR rejected, WireGuard silent drop. (4) **Bug fixes**: `TcpStream::connect_timeout` silently failed on domain names — fixed with `ToSocketAddrs` resolution. `tls_probe` returned "unknown" version — fixed with HEAD request to force handshake. OTR-03 false positive (underscore vs dot in TLS version string) — fixed. ODP-02 false positive (404 classified as vulnerability) — fixed to treat 404 as "rejected". (5) **Tests**: 146→149 (3 new HTTPS/TLS probe tests). |
...
| 2026-07-16 | **Wave 142b — Deep debt sweep + documentation cleanup**: (1) **aws-lc-sys eliminated**: tunnelKeeper switched from reqwest `rustls` to `rustls-no-provider` + explicit `rustls` with `ring` features. `cargo tree -i aws-lc-sys` returns empty — zero C TLS dependencies across all 4 crates. (2) **JsonRpcError typed enum**: `nucleus-deploy` `jsonrpc_uds` returns `Result<String, JsonRpcError>` (6 variants) instead of `Result<_, String>`. `DeployError` derives `#[from] JsonRpcError`. (3) **`as` casts → `try_from`**: `darkforest/check.rs` civil date (3 casts), `crypto/mod.rs` Shannon entropy, `provenance/mod.rs` elapsed_ms. All use `try_from().unwrap_or()` or `usize::from()`. (4) **Clone reduction**: `darkforest/main.rs` eliminates `cli.suite.clone()` via `&str` reference. `nucleus-deploy/spore/mod.rs` iterates owned `Vec<PathBuf>` instead of cloning candidates. `s.clone()` → `s.to_owned()`. (5) **`#[allow]` → `#[expect]`**: `summary.rs` and `verify.rs` annotations evolved with explicit `reason = "..."`. (6) **Production `expect()` removed**: `darkforest/net.rs` TLS_CONFIG uses `Option<Arc<ClientConfig>>` instead of `expect()`. (7) **deny.toml harmonized**: `CDLA-Permissive-2.0` + `Unicode-3.0`/`Unicode-DFS-2016` added to nucleus-deploy and nucleus-primals. `unsafe-libyaml` ban added to nucleus-primals. tunnelKeeper comment updated from "aws-lc-rs backend" to "ring backend". (8) **Test counts**: 245 total (darkforest 149, tunnelKeeper 49, nucleus-deploy 47). tunnelKeeper gained 1 test from ring backend switch. nucleus-primals has 0 tests (not 12 as previously documented). (9) **Documentation**: All root docs updated — stale 256→245 test counts, primalSpring v0.9.36→v0.9.39, biomeOS v3.x→v4.31, Wave headers to 142b. (10) **Cleanup**: `cargo clean` 4 crates (1.7 GB reclaimed), `__pycache__` debris removed. |
| 2026-08-02 | **Workspace evolution — edition 2024 + test advance**: (1) Root workspace `Cargo.toml` unifies all 4 crates (darkforest, tunnelKeeper, nucleus-deploy, nucleus-primals). (2) All crates migrated to Rust edition 2024 via `edition.workspace = true`. (3) `chrono` eliminated from nucleus-deploy — replaced with pure `std::time` helpers. (4) Infrastructure port constants centralized in `nucleus-primals`. (5) **265 Rust tests** (darkforest 149, tunnelKeeper 48+1 ignored, nucleus-deploy 49, nucleus-primals 19). Infrastructure port constants centralized — 8 infra port defaults (JupyterHub, observer, songbird federation, BTSP shadow, RustDesk, membrane HTTP, Forgejo SSH, WireGuard) in `nucleus-primals` with collision tests. Hardcoded port literals replaced across 8 source files. `/home/nobody` fallback evolved to `/tmp/darkforest`. Duplicated slug lists in darkforest tests derived from registry. biomeOS v4.56. |
...
| 2026-08-10 | **Wave 157g ENMESH — composition graph foundation**: (1) **`nucleus-deploy verify --manifest`**: biome.yaml manifest validation against nucleus-primals registry — schema, primal membership, composition kind cross-check (Tower/Nest/Node match registry constants), dependency cycle detection, federation peers. **29/29 PASS** on ironGate manifest. (2) **`nucleus-deploy verify --audit-rpc`**: Vertebrate RPC surface audit — liveness probing, unknown method detection (P0-A health stub pattern), BTSP gating validation. (3) **`biome-irongate.yaml` created**: Canonical toadstool-core v1 schema manifest for ironGate — 13 primals, 3 compositions (tower/nest/node-atomic), dependency graphs, readiness gates, federation peers. (4) **Gossip mesh documented**: TCP 7800 reachable from ironGate to sporeGate/eastGate/strandGate; unreachable to golgi/westGate. (5) **Workload dispatch enrichment**: 28 workload TOMLs enriched with `spring` metadata field for programmatic discovery. (6) **toadStool 157e depot binary**: Still exits after startup (mDNS multicast error on wg0). 12/13 ALIVE, 90 MB RSS. (7) **Cascaded**: biomeOS routing fix, toadStool S375-S379 (manifest convergence, WASM 38/48, Tokio blast radius), cellMembrane golgi CI hook, coralReef dep cleanup. (8) **irongate.toml**: Updated to Wave 157g — binary_generation 157e, biome_yaml_checks 29, gossip section, enmesh status. (9) **274 Rust tests** (58 nucleus-deploy, up from 49). 0 clippy, 0 fmt. |
